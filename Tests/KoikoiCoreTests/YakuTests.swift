...
@Suite struct YakuTests {
    ...
    @Test func gokou() {
        ...
        #expect(yaku(yakus, .fiveBrights)?.points == 10)
        #expect(yaku(yakus, .fourBrights) == nil)
        #expect(yaku(yakus, .rainyFourBrights) == nil)
        #expect(yaku(yakus, .threeBrights) == nil)
    }

    @Test func shikou() {
        let yakus = YakuChecker.checkYaku(captured: cards(0, 8, 28, 44))
        #expect(yaku(yakus, .fourBrights)?.points == 8)
        #expect(yaku(yakus, .fiveBrights) == nil)
        #expect(yaku(yakus, .threeBrights) == nil)
    }
    ...
    @Test func ameShikou() {
        ...
        #expect(yaku(yakus, .rainyFourBrights)?.points == 7)
        #expect(yaku(yakus, .fourBrights) == nil)
        #expect(yaku(yakus, .threeBrights) == nil)
    }
    ...
    @Test func sankou() {
        ...
        #expect(yaku(yakus, .threeBrights)?.points == 5)
    }
    ...
    @Test func hikariExclusivity() {
        ...
        #expect(gokou.count(where: { [.fiveBrights, .fourBrights, .rainyFourBrights, .threeBrights].contains($0.kind) }) == 1)
        // 雨四光時（柳含む4枚）は三光にしない
        let ame = YakuChecker.checkYaku(captured: cards(0, 8, 40, 44))
        #expect(yaku(ame, .rainyFourBrights) != nil)
        #expect(yaku(ame, .threeBrights) == nil)
    }
    ...
    @Test func tane() {
        ...
        #expect(yaku(yakus, .animals)?.points == 1)
    }

    @Test func taneExtra() {
        let yakus = YakuChecker.checkYaku(captured: cards(4, 12, 16, 29, 32, 41))
        #expect(yaku(yakus, .animals)?.points == 2)
    }
    ...
    @Test func taneDisabledByInoshikacho() {
        let yakus = YakuChecker.checkYaku(captured: cards(20, 24, 36, 4, 12))
        #expect(yaku(yakus, .animals) == nil)
        #expect(yaku(yakus, .boarDeerButterfly) != nil)
    }

    @Test func tan() {
        let yakus = YakuChecker.checkYaku(captured: cards(13, 17, 25, 42, 1))
        #expect(yaku(yakus, .ribbons)?.points == 1)
    }
    ...
    @Test func tanDisabledByAkatan() {
        ...
        #expect(yaku(yakus, .ribbons) == nil)
        #expect(yaku(yakus, .redPoetryRibbons)?.points == 7)
    }
    ...
    @Test func totalPoints() {
        ...
        #expect(YakuChecker.totalPoints([Yaku(.fiveBrights, 10), Yaku(.redPoetryRibbons, 5)]) == 15)
    }
}
...
@Suite struct YakuReachTests {
    ...
    @Test func gokouReach() {
        ...
        #expect(reach(reaches, .fiveBrights)?.missing == cards(44))
    }
